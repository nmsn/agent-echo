...
const EDITOR_MAP_MAC = {
    'code': 'code',
    'cursor': 'cursor',
};
const EDITOR_PATH_CHECKS = [
    ['visual studio code', 'code'],
    ['cursor.app', 'cursor'],
];
const AGENT_NAMES_MAC = new Set(['claude']);
function agentCmdlineCheck(cmd) {
    return cmd.includes('claude-code') || cmd.includes('@anthropic-ai');
}
function createPidResolver(startPid) {
    let cached = null;
    return function resolve() {
        ...
        for (let i = 0; i < maxDepth; i++) {
            ...
            if (SYSTEM_BOUNDARY_MAC.has(name))
                break;
            if (TERMINAL_NAMES_MAC.has(name))
                terminalPid = pid;
            lastGoodPid = pid;
            if (!parentPid || parentPid === pid || parentPid <= 1)
                break;
            pid = parentPid;
        }
        cached = {
...
async function main() {
    ...
    const eventTypeFromPayload = (payload.hook_event_name || payload.type);
    ...
    if (eventName === 'UserPromptSubmit') {
        message.content = (payload.text || payload.prompt || payload.content);
